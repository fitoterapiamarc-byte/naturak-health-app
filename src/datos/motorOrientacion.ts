...
function hayCoincidencia(a:string,b:string){const x=canonizar(a),y=canonizar(b);if(x===y)return true;const corta=x.length<=y.length?x:y,larga=x.length>y.length?x:y,p=corta.split(" ").filter(Boolean);return corta.length>=12&&p.length>=3&&larga.includes(corta);}
