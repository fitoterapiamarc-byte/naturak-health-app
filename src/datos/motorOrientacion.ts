...
// Protección estructural: evita que una ampliación futura duplique una ficha
// con el mismo id o exactamente el mismo nombre normalizado.
const idsVistos=new Set<string>();
const nombresVistos=new Set<string>();
export const todasLasCondiciones:Condicion[]=baseCondiciones.filter((condicion)=>{
  const id=normalizarTexto(condicion.id);
  const nombre=normalizarTexto(condicion.nombre);
...
export function orientarPorSintomas(sintomasUsuario:string[]):ResultadoOrientacion[]{
  ...
  return todasLasCondiciones.map(condicion=>{
    const coinc=condicion.sintomas.filter(s=>datos.some(d=>hayCoincidencia(d,s.nombre)));
    const contra=condicion.sintomasQueContradicen.filter(s=>datos.some(d=>hayCoincidencia(d,s.nombre)));
    const alarmas=condicion.sintomasAlarma.filter(a=>datos.some(d=>hayCoincidencia(d,a.nombre)));
    ...
    return{
      ...
      puntuacion,
      confianza:maxima>0?Math.round(puntuacion/maxima*100):0,
      requiereValoracionMedica:coinc.length>0&&alarmas.length>0
    };
  }).filter(r=>r.coincidencias.length>0&&r.puntuacion>0)
    .sort((a,b)=>a.requiereValoracionMedica&&!b.requiereValoracionMedica?-1:!a.requiereValoracionMedica&&b.requiereValoracionMedica?1:b.puntuacion-a.puntuacion);
}